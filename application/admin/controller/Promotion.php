...
//            if ($status !== '-1') {
//                $condition['status'] = $status;
//                $list = $seckill->getPromotionSeckillList($page_index, $page_size, $condition);
//            } else {
//                $list = $seckill->getPromotionSeckillList($page_index, $page_size, $condition);
//            }
//
...
//        $child_menu_list = array(
//            array(
//                'url' => "promotion/getseckillList",
//                'menu_name' => "全部",
//                "active" => $status == '-1' ? 1 : 0
//            ),
//            array(
//                'url' => "promotion/getseckillList?status=0",
//                'menu_name' => "未发布",
//                "active" => $status == 0 ? 1 : 0
//            ),
//            array(
//                'url' => "promotion/getseckillList?status=1",
//                'menu_name' => "进行中",
//                "active" => $status == 1 ? 1 : 0
//            ),
//            array(
//                'url' => "promotion/getseckillList?status=3",
//                'menu_name' => "已关闭",
//                "active" => $status == 3 ? 1 : 0
//            ),
//            array(
//                'url' => "promotion/getseckillList?status=4",
//                'menu_name' => "已结束",
//                "active" => $status == 4 ? 1 : 0
//            )
//        );
//        $this->assign('child_menu_list', $child_menu_list);
//
//        return view($this->style . "Promotion/getseckillList");
//    }
//
//    /**
//     * 添加限时秒杀
...
//
//            $retval = $seckill->addPromotionseckill($seckill_name, $start_time, $end_time, $remark, $goods_id_array, $decimal_reservation_number);
//            return AjaxReturn($retval);
//        }
//        return view($this->style . "Promotion/addSeckill");
//    }
//
//    /**
//     * 修改限时秒杀
...
//            $goods_id_array = request()->post('goods_id_array', '');
//            $decimal_reservation_number = request()->post("decimal_reservation_number", -1);
//            $retval = $seckill->updatePromotionSeckill($seckill_id, $seckill_name, $start_time, $end_time, $remark, $goods_id_array, $decimal_reservation_number);
//            return AjaxReturn($retval);
//        }
//        $info = $this->getSeckillDetail();
//
//        if (! empty($info['goods_list'])) {
//            foreach ($info['goods_list'] as $k => $v) {
//                $goods_id_array[] = $v['goods_id'];
//                $selected_data[$v['goods_id']] = $v['seckill'];
//            }
//        }
//
//
...
//
//        $this->assign("info", $info);
//        return view($this->style . "Promotion/updateSeckill");
//    }
//
//    /**
//     * 获取限时秒杀详情
...
//        if (! is_numeric($seckill_id)) {
//            $this->error("没有获取到秒杀信息");
//        }
//        $seckill = new PromotionService();
//        $detail = $seckill->getPromotionSeckillDetail($seckill_id);
//
//        return $detail;
//    }
    public function getSeckillList()
    ...
    public function getYiFenList()
    {
        ...
        $child_menu_list = array(
            array(
                'url' => "promotion/getyifenList",
                'menu_name' => "全部",
                "active" => $status == '-1' ? 1 : 0
            ),
            array(
                'url' => "promotion/getyifenList?status=0",
                'menu_name' => "未发布",
                "active" => $status == 0 ? 1 : 0
            ),
            array(
                'url' => "promotion/getyifenList?status=1",
                'menu_name' => "进行中",
                "active" => $status == 1 ? 1 : 0
            ),
            array(
                'url' => "promotion/getyifenList?status=3",
                'menu_name' => "已关闭",
                "active" => $status == 3 ? 1 : 0
            ),
            array(
                'url' => "promotion/getyifenList?status=4",
                'menu_name' => "已结束",
                "active" => $status == 4 ? 1 : 0
            )
        );
    ...
    public function closeSeckill()
    {
        ...
        $discount = new PromotionService();
        $res = $discount->closePromotionSeckill($seckill_id);
        return AjaxReturn($res);
    }
    ...
    public function delSeckill()
    {
        ...
        $discount = new PromotionService();
        $res = $discount->delPromotionSeckill($seckill_id);
        return AjaxReturn($res);
    }
