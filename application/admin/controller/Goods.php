...
class Goods extends BaseController
{
    ...
    public function user_other(){
        ...
        $result = $goodservice->get_users_info();

       foreach ($result as $v){
           $goodservice ->add_users($v['user_id'],$v['user_name'],$v['password'],$v['openid'],$v['uname'],$v['parent_id'],$v['headimgurl']);
           //$goodservice ->add_member($v['user_id'],$v['uname']);
           //$goodservice ->add_money($v['user_id'],$v['user_money']);
       }
    ...
    public function backgroundImg(){
        if (request()->isAjax()) {
            ...
            $_SESSION['background_img']  = $background_img;
        }
    }
    ...
    public function addGoods()
    {
        ...
        //获取二维码的背景图
        $background_img = $goods -> getBackgroundImg($goodsId);
        $this->assign("background_img", $background_img);

        //获取不销售的区域
        $no_sale_place = $goods -> getNoSalePlace($goodsId);

        if(!empty($no_sale_place)){
            $no_sale_place_arr = explode(",",$no_sale_place);
            $this->assign("no_sale_place", $no_sale_place_arr);
        }
    ...
    public function getSelectGoodslist()
    {
        ...
        $list = $goods->getSelectGoodsList($page_index, 0, $condition, "sort asc,create_time desc", "goods_id,goods_name,stock,promotion_price,price,seckill_price");
        return $list;
    }
