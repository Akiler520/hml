 ...
 *        
 */
class YifenOrder extends Order
{
    ...
    public function buyNowSession()
    {
        ...
        $cart_list["price"] = 0.01;//$sku_info['pintuan_price'];
     ...
     */
    public function yifenOrderCreate()
    {
        $order = new OrderService();
        $member = new Member();
...
//            $tuangou_group_id = $pintuan->tuangouGroupCreate($this->uid, $receiver_mobile, $goods_sku_list);
        ...
        if (! empty($purchase_restriction)) {
            ...
            $order_id = $pintuan->pintuanOrderCreate(5, $out_trade_no, $pay_type, $shipping_type, $order_from, $buyer_ip, $buyer_message, $buyer_invoice, $shipping_time, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $point, 0, $goods_sku_list, $user_money, $pick_up_id, $shipping_company_id, $coin, $tuangou_group_id);
            $_SESSION['unpaid_goback'] = __URL(__URL__ . "/wap/YifenOrder/orderdetail?orderId=" . $order_id);
            if ($order_id > 0) {
