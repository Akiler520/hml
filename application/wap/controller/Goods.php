...
class Goods extends BaseController
{
    ...
    public function goodsDetail()
    {
        ...
        //$sale_place = $goods -> goods_sale_place($goods_id);


        //获取商品不销售区域
        $no_sale_place = $goods -> goods_no_sale_place($goods_id);

        //获取二维码背景图

        $background_img =  $goods -> getBackgroundImg($goods_id);

        //获取二维码

        $qr_code =  $goods -> getQRCode($goods_id);


        //获取海报

        //$haibao =  $goods -> getHaibao($goods_id);
        ...
        //$this->assign("sale_place", $sale_place);
        $this->assign("no_sale_place", $no_sale_place);
    ...
    public function haibao(){
        if (request()->isAjax()) {
            ...
            if(strlen($goods_name)>60){
                $goods_name1 = substr($goods_name,0,60);
                $goods_name2 = substr($goods_name,60);
            }

            $price= $goods_info['price'];


            $url = __URL('http://www.hmlsm.com/index.php?s=/wap/goods/goodsdetail&id=' . $goods_id.'&source_uid='.$uid);
            $QRcode = getQRcode($url, 'upload/goods_uid_qrcode', 'goods_qrcode_' . $goods_id.'_'.$uid);


            //生成海报开始
            if(!empty($background_img) && !empty($QRcode)){
                $path1 = "public/static/images/back_ground_kong.png";

                $dst_path = $path1;//背景图片路径
                $src_path = $background_img;//覆盖图
                //创建图片的实例
                ...
                imagecopymerge($dst, $src, 0, 146, 0, 0, $dst_w, 792, 100);
                ...
                list($dst_w, $dst_h) = getimagesize($dst_path);
                //将覆盖图复制到目标图片上，最后个参数100是设置透明度（100是不透明），这里实现不透明效果
                imagecopymerge($dst, $src, 10, 938, 0, 0, 180, 180, 100);
                ...
                $red = imagecolorallocate($dst, 255, 0x00, 0x00);//字体颜色
                imagefttext($dst, 16, 0, 200, $dst_h-160, $black, $font, '长按二维码查看详情');


                if(strlen($goods_name)>60){
                    imagefttext($dst, 20, 0, 10, 40, $black, $font, $goods_name1);
                    imagefttext($dst, 20, 0, 10, 100, $black, $font, $goods_name2);
                }
                else{
                    imagefttext($dst, 20, 0, 10, 40, $black, $font, $goods_name);
                }

                imagefttext($dst, 20, 0, 200, $dst_h-100, $red, $font, "￥ ".$price);

                //imagefttext($dst, 16, 0, 200, $dst_h-120, $black, $font, '团团购');
                //输出图片
                imagepng($dst,'upload/goods/haibao/haibao_'.$goods_id.'_'.$uid.'.png');
                imagedestroy($dst);
                $haibao = 'upload/goods/haibao/haibao_'.$goods_id.'_'.$uid.'.png';
            }
            //生成海报结束
...
//    }
    ...
    public function tjcx()
    {
        if (request()->isAjax()) {
            ...
            $this->assign("title_before", '特价促销');
            return view($this->style . 'Goods/tjcx');
        }
    }

    public function rmsp()
    {
        if (request()->isAjax()) {
            $page_index = request()->get('page', '1');
            ...
            $this->assign("title_before", '热卖商品');
            return view($this->style . 'Goods/rmsp');
        }
    }
    public function xpss()
    {
        if (request()->isAjax()) {
            $page_index = request()->get('page', '1');
            $group_id = request()->get("group_id", "3");
            ...
            $this->assign("title_before", '新品上市');
            return view($this->style . 'Goods/xpss');
        }
    }
