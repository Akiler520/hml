...
//        if (request()->isMobile() && strpos($_SERVER['HTTP_USER_AGENT'],'MicroMessenger') !== false) {
    ...
    public function send()
    {
        ...
        $token = $this->getToken($weixin_AppId,$weixin_AppSecret);



        //发送的文章内容
        $content = '亲，拼团开始了，就差您一位了，快去参与吧！点击链接参与拼团：http://www.hmlsm.com/index.php?s=/wap/goods/goodsdetail&id='.$goods_id;


        $data = '{
                    
                    "touser":"oEHd00k-9LKDitV_sGA23RN76M0M",
   ...
   public function shareGoods()
    {
        ...
        //获取所有的用户微信openid
        $member = new member();
        $user_openid = $member -> getUserOpenid();


        /*$pattern = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLOMNOPQRSTUVWXYZ';
        $str = $pattern{mt_rand(0,52)};
        $num=mt_rand(0,9);


        if(!empty($user_openid)){
            foreach ($user_openid as $info){
                $info =json_decode($info,true);
                if(strpos($info['wx_openid'],$str)!=false && strpos($info['wx_openid'],$num)!=false ){
                    $res[] = $info;
                }
            }
        }*/
