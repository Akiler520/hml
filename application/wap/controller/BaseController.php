...
class BaseController extends Controller
{
    ...
    public function wchatLogin()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') && $bind_status == false) {
            ...
            //获取会员的上级id
            $parent_id = request()->get('source_uid', '')==""?0:request()->get('source_uid', '');
                ...
                if (! empty($token['unionid'])) {
                    ...
                    if ($retval == 1) {
                        ...
                        if ($retval == USER_NBUND) {
                            ...
                            $result = $this->user->registerMember('', '123456', '', '', '', '', $token['openid'], $info, $wx_unionid,$parent_id);
                            if ($result) {
                                // 注册成功送优惠券
    ...
    public function getShareContents()
    {
            ...
            switch ($flag) {
                case "shop":
                    if (! empty($this->uid)) {
                        ...
                        $current_user = "分享人1：" . $user_info["nick_name"];
                    } else {
                ...
                case "qrcode_shop":
                    ...
                    $current_user = "分享人2：" . $user_info["nick_name"];
                    break;
                case "qrcode_my":
                    ...
                    $current_user = "分享人3：" . $user_info["nick_name"];
                    break;
                case "bargain":
                    ...
                    $current_user = "分享人4：" . $user_info["nick_name"];
                    break;
                case "cms":
                    ...
                    $current_user = "分享人5：" . $user_info["nick_name"];
                    break;
            }
        } else {
            if (! empty($this->uid)) {
                ...
                $current_user = "分享人6：" . $user_info["nick_name"];
            } else {
        ...
        switch ($flag) {
            ...
            case "goods":
                ...
                //判断商品是否为拼团商品
                $is_tuangou = $goods ->getGoodsTuangou($goods_id);
                if($is_tuangou==1){
                    $share_content["share_title"] ='【拼团】'. $goods_detail["goods_name"];
                }
                else{
