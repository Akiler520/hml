...
//                     }
     ...
     * @param string $field            
     */
    public function getwancheng($condition = '', $field = '*')
    {
        $sum = Db::table('nfx_commission_distribution')->where($condition)
        ->sum($field);
        if(empty($sum))
        {
            return 0;
     ...
     * @param string $field            
     */
    public function get_shop_id($condition = '', $field = '*')
    {
        $info = Db::table('nfx_promoter')->where($condition)
            ->field($field)
            ->find();
        return $info;
    }
