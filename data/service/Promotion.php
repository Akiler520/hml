...
class Promotion extends BaseService implements IPromotion
{
     ...
     */
    public function addPromotionseckill($seckill_name, $start_time, $end_time, $remark, $goods_id_array, $decimal_reservation_number)
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $promotion_seckill->startTrans();
        try {
            ...
            $promotion_seckill->save($data);
            $seckill_id = $promotion_seckill->seckill_id;
            $this->addUserLog($this->uid, 1, '营销', '限时秒杀', '添加限时秒杀：'.$seckill_name);
            $goods_id_array = explode(',', $goods_id_array);
            $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
            $promotion_seckill_goods->destroy([
            ...
            foreach ($goods_id_array as $k => $v) {
                ...
                $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
                $seckill_info = explode(':', $v);
                $goods_seckill = new GoodsSeckill();
                $count = $goods_seckill->getGoodsIsSeckill($seckill_info[0], $start_time, $end_time);
                // 查询商品名称图片
                if ($count > 0) {
                    $promotion_seckill->rollback();
                    return ACTIVE_REPRET;
                }
                $goods = new NsGoodsModel();
                $goods_info = $goods->getInfo([
                    'goods_id' => $seckill_info[0]
                ], 'goods_name,picture');
                $data_goods = array(
                    'seckill_id' => $seckill_id,
                    'goods_id' => $seckill_info[0],
                    'seckill' => $seckill_info[1],
                    'status' => 0,
                    'start_time' => getTimeTurnTimeStamp($start_time),
                    'end_time' => getTimeTurnTimeStamp($end_time),
                    'goods_name' => $goods_info['goods_name'],
                ...
                $promotion_seckill_goods->save($data_goods);
            }
            $promotion_seckill->commit();
            return $seckill_id;
        } catch (\Exception $e) {
            $promotion_seckill->rollback();
            return $e->getMessage();
     ...
     * @see \data\api\IPromote::updatePromotionDiscount()
     */
    public function updatePromotionSeckill($seckill_id, $seckill_name, $start_time, $end_time, $remark, $goods_id_array, $decimal_reservation_number)
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $promotion_seckill->startTrans();
        try {
            ...
            $this->addUserLog($this->uid, 1, '营销', '限时秒杀', '修改限时秒杀：'.$seckill_name);
            $goods_id_array = explode(',', $goods_id_array);
            $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
            $promotion_seckill_goods->destroy([
                'seckill_id' => $seckill_id
     ...
     * @see \data\api\IPromote::closePromotionDiscount()
     */
    public function closePromotionSeckill($seckill_id)
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $promotion_seckill->startTrans();
        try {
            $retval = $promotion_seckill->save([
                'status' => 3
            ], [
                'seckill_id' => $seckill_id
            ]);
            if ($retval == 1) {
                ...
                $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
                $retval = $promotion_seckill_goods->save([
                    'status' => 3
                ], [
                    'seckill_id' => $seckill_id
                ]);
 ...
 */
     ...
     * @see \data\api\IPromote::getPromotionDiscountList()
     */
    public function getPromotionSeckillList($page_index = 1, $page_size = 0, $condition = '', $order = 'create_time desc')
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $list = $promotion_seckill->pageQuery($page_index, $page_size, $condition, $order, '*');
        foreach($list['data'] as $v ){
     ...
     * (non-PHPdoc)
     *
     * @see \data\api\IPromote::getPromotionSeckillDetail()
     */
    public function getPromotionSeckillDetail($seckill_id)
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $promotion_detail = $promotion_seckill->get($seckill_id);
        $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
        $promotion_goods_list = $promotion_seckill_goods->getQuery([
            'seckill_id' => $seckill_id
        ], '*', '');
     ...
     * @see \data\api\IPromote::delPromotionDiscount()
     */
    public function delPromotionSeckill($seckill_id)
    {
        $promotion_seckill = new NsPromotionSeckillModel();
        $promotion_seckill_goods = new NsPromotionSeckillGoodsModel();
        $promotion_seckill->startTrans();
        try {
            $seckill_id_array = explode(',', $seckill_id);
            foreach ($seckill_id_array as $k => $v) {
                $promotion_detail = $promotion_seckill->get($seckill_id);
                if ($promotion_detail['status'] == 1) {
                    $promotion_seckill->rollback();
                    return - 1;
                }
                $promotion_seckill->destroy($v);
                $promotion_seckill_goods->destroy([
                    'seckill_id' => $v
                ]);
            }
