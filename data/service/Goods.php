...
use data\model\SysMemberAccount;
...
use data\model\HhsUsers;
use data\model\SysUser;
use data\model\SysMember;
...
class Goods extends BaseService implements IGoods
{
    ...
    public function get_users_info(){
        $users_info = new HhsUsers();

        $row = $users_info->getQuery('', 'user_id,user_name,password,user_money,parent_id,openid,headimgurl,uname','');
    ...
    public  function add_users($uid,$user_name,$user_password,$wx_openid,$nick_name,$parent_id,$user_headimg)
    {
        ...
        $users_info = new SysUser();
        $res = $users_info->save($data_goods);
        return $res;
    }


    public  function add_member($uid,$nick_name)
    {
        ...
        $users_info = new SysMember();
        $res = $users_info->save($data_goods);
        return $res;
    }
    public function add_money($user_id,$user_money){
        $data_goods = array(
            'uid' => $user_id,
            'balance' => $user_money
        );

        $users_info = new SysMemberAccount();
        $res = $users_info->save($data_goods);
        return $res;
    }
    ...
    public function addOrEditGoods($goods_id, $goods_name, $shopid, $category_id, $category_id_1, $category_id_2, $category_id_3, $supplier_id, $brand_id, $group_id_array, $goods_type, $market_price, $price, $cost_price, $point_exchange_type, $point_exchange, $give_point, $is_member_discount, $shipping_fee, $shipping_fee_id, $stock, $max_buy, $min_buy, $min_stock_alarm, $clicks, $sales, $collects, $star, $evaluates, $shares, $province_id, $city_id, $picture, $keywords, $introduction, $description, $QRcode, $code, $is_stock_visible, $is_hot, $is_recommend, $is_new, $sort, $image_array, $sku_array, $state, $sku_img_array, $goods_attribute_id, $goods_attribute, $goods_spec_format, $goods_weight, $goods_volume, $shipping_fee_type, $extend_category_id, $sku_picture_values, $virtual_goods_type_data, $production_date, $shelf_life, $ladder_preference, $goods_video_address, $pc_custom_template, $wap_custom_template, $max_use_point, $is_open_presell, $presell_delivery_type, $presell_price, $presell_time, $presell_day, $goods_unit, $discount_info, $decimal_reservation_number,$background_img,$no_sale_place)
    {
        ...
        $haibao ="";
        //生成海报开始
       /*if(!empty($background_img) && !empty($QRcode)){
           $path1 = "public/static/images/kongbai.png";

           $dst_path = $background_img;//背景图片路径
           $src_path = $path1;//覆盖图
           //创建图片的实例
           ...
           imagecopymerge($dst, $src, 0, $dst_h-200, 0, 0, $dst_w, 200, 100);


           header("Content-type: image/png");
           imagepng($dst,'upload/goods/haibao/'.date("Ymd").'.png');//根据需要生成相应的图片
           imagedestroy($src);



           $dst_path = 'upload/goods/haibao/'.date("Ymd").'.png';//背景图片路径
           $src_path = $QRcode;//覆盖图
           //创建图片的实例
           $dst = imagecreatefromstring(file_get_contents($dst_path));
           $src = imagecreatefromstring(file_get_contents($src_path));
           ...
           list($dst_w, $dst_h) = getimagesize($dst_path);
           //将覆盖图复制到目标图片上，最后个参数100是设置透明度（100是不透明），这里实现不透明效果
           imagecopymerge($dst, $src, 10, $dst_h-190, 0, 0, 180, 180, 100);

           //如果覆盖图图片本身带透明色，则使用imagecopy方法

           header("Content-type: image/png");
           imagepng($dst,'upload/goods/haibao/'.date("Ymd").$goods_id.'.png');//根据需要生成相应的图片
           imagedestroy($src);

           $dst_path = 'upload/goods/haibao/'.date("Ymd").$goods_id.'.png';
           //创建图片的实例
           $dst = imagecreatefromstring(file_get_contents($dst_path));
           list($dst_w, $dst_h) = getimagesize($dst_path);

           //打上文字
           $font = 'public/static/font/simsun.ttc';//字体路径

           $black = imagecolorallocate($dst, 0x00, 0x00, 0x00);//字体颜色
           imagefttext($dst, 16, 0, 200, $dst_h-160, $black, $font, '长按二维码查看详情');
           imagefttext($dst, 16, 0, 200, $dst_h-120, $black, $font, '团团购');
           //输出图片
           imagepng($dst,'upload/goods/haibao/haibao'.$goods_id.'.png');
           imagedestroy($dst);
           $haibao = 'upload/goods/haibao/haibao'.$goods_id.'.png';
       }*/
         //生成海报结束

        $no_sale_place = substr($no_sale_place,0,-1);
...
//             array_multisort($arrSort[$sort['field']], SORT_ASC, $spec_list); 
    ...
    public function getGoodsTuangou($goods_id){
        $tuangou = new NsPromotionTuangouModel();

        $is_tuangou = 0;
        $tuangou_info = $tuangou->getInfo([
            "goods_id" => $goods_id

        ], "*");
        if (! empty($tuangou_info)) {
           if($tuangou_info['is_open'] ==1){
               $is_tuangou = 1;
           }
        }
        return $is_tuangou;
    ...
    public function goods_sale_place($goods_id){
        $place = '';
        $model = new NsGoodsModel();
        $info = $model->getInfo([
            'goods_id' => $goods_id
        ...
        if($info['province_id'] !==0 ){
            ...
            $province_info = $province_model->getInfo([
                'province_id' => $info['province_id']
            ], "province_name");

            $city_model = new City;
            $city_info = $city_model->getInfo([
                'city_id' => $info['city_id']
            ], "city_name");

            $place = $province_info['province_name'] . $city_info['city_name'];
    ...
    public function goods_no_sale_place($goods_id){
    ...
    //获取二维码的背景图
    public function getBackgroundImg($goods_id){
        $res = '';
        $model = new NsGoodsModel();
        $info = $model->getInfo([
            'goods_id' => $goods_id
        ...
        if(!empty($info['background_img'] )){
            $res = $info['background_img'];
        }
    ...
    //获取不销售的区域
    public function getNoSalePlace($goods_id){
        $res = '';
        $model = new NsGoodsModel();
        $info = $model->getInfo([
            'goods_id' => $goods_id
        ...
        if(!empty($info['no_sale_place'] )){
            $res = $info['no_sale_place'];
        }
    ...
    //获取二维码的背景图
    public function getQRCode($goods_id){
        $res = '';
        $model = new NsGoodsModel();
        $info = $model->getInfo([
            'goods_id' => $goods_id
    ...
    //获取海报
    public function getHaibao($goods_id){
        $res = '';
        $model = new NsGoodsModel();
        $info = $model->getInfo([
            'goods_id' => $goods_id
