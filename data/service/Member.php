...
use data\model\NsFanliModel as NsFanliModel;
...
public function registerMember($user_name, $password, $email, $mobile, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid,$parent_id=0)
    {
        ...
        $res = parent::add($user_name, $password, $email, $mobile, 0, $user_qq_id, $qq_info, $wx_openid, $wx_info, $wx_unionid, 1,0,$parent_id);
        if ($res > 0) {
            // 获取默认会员等级id
...
//会员返利
    public function yjfl($start_date,$end_date){
        $condition["add_date"] = [
            [
                ">",
                $start_date
        ...
        $condition['fl_state'] = 1;
        // 查询返利
        $fanli_model = new NsFanliModel();
        $fanli_list = $fanli_model->getQuery( $condition, '*','add_date desc');
        if(!empty($fanli_list)){
            foreach ($fanli_list as $vo){

                $member_account = new MemberAccount();
                $result =$member_account->addFl($vo['parent_id'],$vo['fanli_money'],$vo['fl_id']);
    ...
    public function getUserOpenid(){
        ...
        $condition = array(
            ...
            $list = $user->getQuery($condition, 'wx_openid', '');
