...
class Order extends BaseService implements IOrder
{
   ...
   /* public function getOrderInfo($order_id)
    ...
    public function getShopOrderList($buyer_id)
    {
        $order_model = new NsOrderModel();
        // 查询主表
    ...
    //根据订单编号查询商品id
    public function getGoodsId($out_trade_no){
        $res=0;
        $order_model = new NsOrderModel();
        ...
        if (!empty($order_info)) {
            if ($order_info['order_id'] != 0) {
                ...
                $order_goods_info = $order_goods_model->getInfo([
                    'order_id' => $order_info['order_id']
                ], 'goods_id');
                if(!empty($order_goods_info)){
                    $res = $order_goods_info['goods_id'];
                }
     ...
     */
    public function orderCreateBragain($out_trade_no, $goods_sku_list, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $bragain_money, $uid)
    {
        $order = new OrderBusiness();
        $retval = $order->orderCreateBragain($out_trade_no, $goods_sku_list, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $bragain_money, $uid);
