...
//             }
    ...
    public function getNfxUserAccount($uid, $shop_id)
    {
		...
		//2018年8月19日李发柏修改---解决会员购买后推广员无法看见还没有收货的提成
		//查询nfx_commission_distribution表中is_issue为0的值,这里需要注意shop_id可能为0需要单独查询下表nfx_promoter
		
		$get_shop_id = $account_model->get_shop_id([
            'uid' => $uid
        ], 'promoter_id');
		
		
		$account_wancheng = $account_model->getwancheng([
		    'is_issue' => 0,
            'promoter_id' => $get_shop_id['promoter_id']
        ], 'commission_money');
		
		$account_data['commission_wancheng']=$account_wancheng;
