...
$(function() {
	...
	$(".js-addMenuBtn").live("click",function() {
		...
		html += '<a href="javascript:void(0);" ondragstart="return false" class="pre_menu_link jsMenu" data-menuid="-1" data-pid="0" data-menu-eventurl="" data-menu-type="1" data-detault-menu-type="1" data-mediaid="0" data-sort="'+currentIndex+'" data-appid="" data-pagepath="">';
	...
	// 修改网址
	$("#urltext,#smallProgramUrltext").blur(function(){
		if(parseInt($(this).attr("data-flag")) == 1 && $(this).val().length > 0){
			var obj = getCurrentMenu();
			var isUpdate = true;//是否更新
			//没有变化，则做不修改操作
	...
	$("#appid,#pagepath").blur(function(){
		var type = $(this).attr("id");
		var val = $(this).val();
		var obj = getCurrentMenu();
		if((type == 'appid' && obj.attr("data-appid") == val) || (type == 'pagepath' && obj.attr("data-pagepath") == val)){
			return;
		}
		$.ajax({
			url : __URL(ADMINMAIN + "/wchat/updateMenuSmallProgramConfigAjax"),
			type : "post",
			async : false,
			data : { "menu_id" : obj.attr("data-menuid"), "type" : type, "val" : val},
			success : function(res){;
				if(res>0){
					var obj = getCurrentMenu();
					if(type == 'appid'){
						obj.attr("data-appid",val);
					}else if(type == 'pagepath'){
						obj.attr("data-pagepath",val);
					}
					showDialog("修改成功", 1);
				}else{
					showDialog("修改失败", 0);
	 ...
	 * 编辑菜单内容 点击跳转小程序
	 */	
	$(".js_small_program").click(function(){
		$(this).addClass("selected").siblings().removeClass("selected");
		$("#edit").hide();
		$("#url").hide();
		$("#small_program").show();
...
//			showDialog("请先按要求编辑完当前操作！！！", 0);
	 ...
	 * 网址验证，可以为空
	 */
	$("#urltext,#smallProgramUrltext").keyup(function(){
		...
		if($(this).next().find("li.jsSubMenuInner").length){
			...
			$(this).next().find("li.jsSubMenuInner").each(function(){
				if($(this).hasClass("current") && $(".js_radio_sendMsg").hasClass("selected") && ($(this).children("a").attr("data-mediaid") == undefined || $(this).children("a").attr("data-mediaid") == "" || parseInt($(this).children("a").attr("data-mediaid")) == 0)){
					...
					if(parseInt($(this).children("a").attr("data-menu-type")) == 4 && ($(this).children("a").attr("data-menu-eventurl") == undefined || $(this).children("a").attr("data-menu-eventurl") == "")){
						flag = true;
						msg = "跳转网址不能为空";
						menuClick(menu_self);
						subMenuClick(this);
					...
					if(parseInt($(this).children("a").attr("data-menu-type")) == 4 && ($(this).children("a").attr("data-appid") == undefined || $(this).children("a").attr("data-appid") == "")){
						flag = true;
						msg = "小程序appid不能为空";
						menuClick(menu_self);
						subMenuClick(this);
					...
					if(parseInt($(this).children("a").attr("data-menu-type")) == 4 && ($(this).children("a").attr("data-pagepath") == undefined || $(this).children("a").attr("data-pagepath") == "")){
						flag = true;
						msg = "小程序页面路径不能为空";
						menuClick(menu_self);
						subMenuClick(this);
			...
			if($(this).parent().hasClass("current") && $(".js_radio_sendMsg").hasClass("selected") && ($(this).attr("data-mediaid") == undefined || $(this).attr("data-mediaid") == "" || parseInt($(this).attr("data-mediaid")) == 0)){
				...
				if(parseInt($(this).attr("data-menu-type")) == 4 && ($(this).attr("data-menu-eventurl") == undefined || $(this).attr("data-menu-eventurl") == "")){
					flag = true;
					msg = "跳转网址不能为空";
					menuClick(menu_self);
					return false;
				...
				if(parseInt($(this).attr("data-menu-type")) == 4 && ($(this).attr("data-appid") == undefined || $(this).attr("data-appid") == "")){
					flag = true;
					msg = "小程序appid不能为空";
					menuClick(menu_self);
					return false;
				...
				if(parseInt($(this).attr("data-menu-type")) == 4 && ($(this).attr("data-pagepath") == undefined || $(this).attr("data-pagepath") == "")){
					flag = true;
					msg = "小程序页面路径不能为空";
					menuClick(menu_self);
					return false;
...
function menuClick(obj){
	...
	if(parseInt($(obj).attr("data-menu-type")) == 2 || parseInt($(obj).attr("data-menu-type")) == 3){
		...
		$("#url").hide();
		$("#small_program").hide();
	}else if(parseInt($(obj).attr("data-menu-type")) == 4){
		$(".js_small_program").addClass("selected").siblings().removeClass("selected");
		$("#edit").hide();
		$("#url").hide();
		$("#small_program").show();
	}else{
	...
	var menu_event_url = $(obj).attr("data-menu-eventurl");//菜单url
	var appid = $(obj).attr("data-appid");//菜单url
	var pagepath = $(obj).attr("data-pagepath");//小程序跳转页路径
	$(".js_titleNolTips").text(DEFAULT_MENU_TIP);//汉字或16个字母
	...
	$("#smallProgramUrltext").val(menu_event_url);
	$("#appid").val(appid);
...
function subMenuClick(obj){
	...
	var appid = $(obj).attr("data-appid");//小程序appid
	var pagepath = $(obj).attr("data-pagepath");//小程序跳转页路径
