...
function initPageData(page_count,page_size,total_count){
	...
	$(".total-data").text("共"+total_count+"条");
	$(".total-data").attr("data-total-count",total_count);
	$(".page-count").text("共"+page_count+"页");
	if(page_count > 5){
		$("#JslastPage").html('<span class="ellipsis">···</span><a onclick="JumpForPage(this)">'+page_count+'</a>').css("display","inline-block");
	}
}
...
function pagenumShow(pageindex,pagecount,pageshow){
	...
	$("#skipPage").attr("data-curr-page", pageindex);
